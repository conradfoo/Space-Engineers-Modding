...
namespace Economy
{
  ...
  public class MessageIncomingMessageParts : MessageBase
  {
	...
	[ProtoContract]
	public class MessageChangeBalance : MessageBase
	{
		[ProtoMember(1)]
		...
		public override void ProcessServer()
		{
			//Change balance of account playerID by amount.
			BankSystem.Instance.currentState.playerAccounts[playerID] += amount;
		}
	}
	
	[ProtoContract]
	public class MessageGetBalanceRequest : MessageBase
	{
		[ProtoMember(1)]
		public long playerID;
		...
		public override void ProcessServer()
		{
			MessageUtils.SendMessageToAllPlayers(new MessageSendBalance(){ balance = BankSystem.Instance.currentState.playerAccounts[playerID], EntityId = RequestingEntityId });
		}
	}
	
	[ProtoContract]
	public class MessageSendBalance : MessageBase
	{
		[ProtoMember(1)]
		...
		public override void ProcessClient()
		{
			//string msg = "Your current balance is " + balance.ToString();
			//MyAPIGateway.Utilities.ShowNotification(msg, 5000);
			IMyEntity block;
			if (MyAPIGateway.Entities.TryGetEntityById(EntityId, out block))
			{
              block.GameLogic.GetAs<TradingTerminal>().balance = balance;
			  Log.Info("Balance sent to");
			  Log.Info(EntityId.ToString());
			}
		}
	...
	[ProtoContract]
	public class MessageGetPriceRequest : MessageBase
	{
		[ProtoMember(1)]
		public long RequestingEntityId;
		...
		public override void ProcessServer()
		{
			MessageUtils.SendMessageToAllPlayers(new MessageSendPrice(){ EntityId = RequestingEntityId, price = BankSystem.Instance.currentState.priceList[itemId] });
		}
	}
	
	[ProtoContract]
	public class MessageSendPrice : MessageBase
	{
		[ProtoMember(1)]
		public long EntityId;
		...
		public override void ProcessClient()
		{
			...
			if (MyAPIGateway.Entities.TryGetEntityById(EntityId, out block))
			{
				block.GameLogic.GetAs<TradingTerminal>().price = price;
				Log.Info("Price sent to");
				Log.Info(EntityId.ToString());
				MyAPIGateway.Utilities.ShowNotification("Price update.",5000);
			}
		}
	...
	[XmlInclude(typeof(MessageChangeBalance))]
	[XmlInclude(typeof(MessageGetBalanceRequest))]
	[XmlInclude(typeof(MessageSendBalance))]
	[XmlInclude(typeof(MessageIncomingMessageParts))]
	[XmlInclude(typeof(MessageGetPriceRequest))]
	[XmlInclude(typeof(MessageSendPrice))]
